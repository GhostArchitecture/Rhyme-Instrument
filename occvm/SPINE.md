...
| **P-6** | **The carrier phase — which of its constants this model can actually take.** Registered at 2.39, when the owner asked for "a constant for the liquid". The build plan has said since step 3 that a lava lamp is two phases in a heat-driven density race; the model has carried one phase, the wax, and the carrier entered only as `0.0567` typed as a **default argument** inside `globules.js` `buoyantStress` — reachable by no other consumer, owned by nothing, and invisible to the token auditor because a fallback in a signature is not a token. | Three quantities, and they separate cleanly. **Δρ/ρ** reaches `buoyantStress` and `risesAt`, i.e. whether anything in the field can rise. **γ_wc**, the wax/carrier interfacial tension, would replace the wax/air γ in **both arrest lengths**, since each is γ over a stress. **η_c** would set a rise speed and a merge rate. | For Δρ: two real densities instead of an inverted ratio. For γ_wc: a measurement, or a defensible estimation route. For η_c: a consumer that does not already have its magnitude authored. | **Closed at one pass, with one constant adopted and two refused, each for a different reason.** **Δρ/ρ is adopted as `CARRIER.contrast` and is FREE, not measured** — it is inverted from the authored 30 px ceiling, since √(γ/(Δρ·g)) = 30 px exactly at 0.0567, so the ceiling fixes the contrast rather than the reverse. It ships at the value it already had: secondary sources bracket a real lamp at 0.022–0.056, a **smaller** contrast is **further** from τ₀ rather than nearer, and moving into the bracket would change no rendered pixel while trading a stated inversion for a secondary decimal — the trade this system refused for γ at 2.10. **γ_wc is refused, and for a worse reason than P-1's:** P-1 closes γ as unmeasurable; γ_wc for *this* substance is **ill-posed**, because the substance is an aqueous matrix and the lamp analogy's carrier is aqueous, so the two are not immiscible and there is no interface to have a tension. **η_c is refused as D12:** the rise speed is authored precisely because the substance says buoyancy is zero, so a derivation feeding it would have no consumer. **The direction is the finding.** Every carrier lever points *away* from a more merged field: both arrest lengths scale linearly in γ, so a lower liquid/liquid tension shortens them and the field arrests **more** — at γ 0.010 a 9+9 pair reads *barely joined* rather than *dumbbell*. Completion would need γ to **rise** to 0.0635 N/m for a 9+9 and 0.2115 for a 30+30, and a liquid/liquid interface is the opposite direction. Reopens only if a wax/carrier pair is ever named that is genuinely immiscible with this substance. |
