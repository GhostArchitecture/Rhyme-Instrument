...
test("2.3 — .slab wears the material, and the material predicts two of the three", () => {
  /* This tool is the FIRST surface in either repository to wear OCCVM-L12 rather than the hand. It was
   * chosen because its substrate carries no win/lose meaning; BTC's sits under the outcome colours L5
   * governs, which is not where a new derivation gets proven. */
  const fs = require("fs");
  const M = require(path.join(ROOT, "occvm", "material.js"));
  const css = fs.readFileSync(path.join(ROOT, "occvm", "substrate.css"), "utf8");
  const tok = n => (css.match(new RegExp("--" + n + ":\\s*(#[0-9a-f]{6})")) || [])[1];

  /* the un-fitted prediction: only `body` was anchored, to L1's floor. The other two fall out of
     angular Fresnel at L2's cut geometry and land on the authored values exactly. */
  assert.equal(M.ARAGONITE.body, "#0e0d13", "body is anchored to L1's substrate floor");
  assert.equal(tok("m-sub-hi"), "#2c2a36", "--m-sub-hi must predict the authored --sub-hi to the byte");
  assert.equal(tok("m-sub-lo"), "#0e0d13", "--m-sub-lo is the anchor itself");
  assert.notEqual(tok("m-sub"), "#1b1a22", "--m-sub is where the material and the hand disagree");

  /* the slab actually consumes them — an adopted surface that still reads the authored tokens has
     adopted nothing, and every JS assertion above would still pass. */
  ...
  assert.ok(/var\(--m-sub-hi\)/.test(slab) && /var\(--m-sub\)/.test(slab) && /var\(--m-sub-lo\)/.test(slab),
    ".slab must read the material tokens, not the authored --sub-*");
  assert.ok(!/var\(--sub-hi\)|var\(--sub\)[^-]/.test(slab), ".slab must not still read the authored ramp");

  /* provenance: the file is generated, and a hand-edit must be catchable */
  assert.ok(/GENERATED by occvm\/tools\/gen-substrate\.js/.test(css) && /DO NOT HAND-EDIT/.test(css));
  const r = require("child_process").spawnSync(process.execPath,
    [path.join(ROOT, "occvm", "tools", "gen-substrate.js"), "--check"], { encoding: "utf8" });
  assert.equal(r.status, 0, "substrate.css must match what material.js derives: " + (r.stderr || r.stdout));
});
