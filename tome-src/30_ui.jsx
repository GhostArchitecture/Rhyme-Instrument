...
/* ---- the ambient floor (OCCVM-L13) -----------------------------------------------------------
   THE FLOOR MOVED OUT AT 2.31. Every line of it — the buoyancy cycle, the coil, the arrest, the
   metaball threshold, P-3's linear bridge and all six authored constants — is `occvm/floor.js`,
   spliced into this tool's engine as `OCCVM_FLOOR` and into BTC's page alongside it. The reason is
   L13's amendment: BTC Terminal is granted the floor on its page ground, so two tools run one
   behaviour and one owner is the whole of L3. The comment that used to sit here said "a shared part
   must not carry what one tool is withheld", and that was true of the law as it stood; the law moved
   and the code followed it.
   What did NOT move is the ten lines below. `useAmbientFloor` is React lifecycle, and OCCVM's parts
   are CSS and framework-free JS — splicing a hook into occvm/ would make React a dependency of every
   conforming tool, which is a claim nobody has made. The record for the floor itself, and every
   measurement behind its constants, is in the part. */
